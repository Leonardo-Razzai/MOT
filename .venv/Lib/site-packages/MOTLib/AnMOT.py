...
def NumOfAtoms_from_fit(V0, dV0):
    """
    Calculate the number of atoms from V0 parameter of the Lorentzian fit.
    ...
    Na = P / (fractional_sigma * Decay_rate * Ep)
...
def NumOfAtoms(V0, dV0, Delta, Gamma):
    """
    Calculate the number of atoms from fluorescence signal V0 measured at detunig Delta.
...
class MOTdata_Lorentzian():
    ...
    def SelectData(self, tmin = 3.8, tmax = 3.9, plot = False):
        ...
        for i, D in enumerate(self.BN_vals):
            ...
            for delta_time in np.linspace(min_delta_time, max_delta_time, 20):
                ...
                x_picked_list.append(np.mean(x_interval))
                x_picked_err_list.append(np.std(x_interval))
                y_picked_list.append(np.mean(y_interval))
                y_picked_err_list.append(np.std(y_interval))
            
            x_picked_list = np.array(x_picked_list)
            x_picked_err_list = np.array(x_picked_err_list)
            y_picked_list = np.array(y_picked_list)
            y_picked_err_list = np.array(y_picked_err_list)
            
            x_picked = np.average(x_picked_list, weights=1/x_picked_err_list**2)
            y_picked = np.average(y_picked_list, weights=1/y_picked_err_list**2)
            err_y_picked = np.sum(1/y_picked_err_list**2)**(-0.5)
            self.PHOTOEMISSION_TIMES.append(x_picked)
            self.PHOTOEMISSION_VALS.append(y_picked)
            self.ERRORS[i] = self.ERRORS[i] + err_y_picked
            
            if plot:
                
                index_plot = (x_data > x_picked - 50*min_delta_time) * (x_data < x_picked + 50*min_delta_time)
    ...
    def FitLorentzian(self, plot = False):
        ...
        if plot:
            ...
            ax.set_title(f'Fit of fluorescence signal vs Cooler Beat-Note B={self.B}', fontdict=title_font)
    ...
    def CalculateNumOfAtoms(self):
        """
        Calculate the number of atoms from the Lorentzian fit parameter V0.
        """

        Na, dNa = NumOfAtoms_from_fit(self.V0, self.dV0)
                  
        print(f'Number of atoms = ({Na/1e7:.2f} {pm} {dNa/1e7:.2f}) * 10^7')
...
class MOTdata_Loading():
    ...
    def SelectData_Peaks(self, tmin = 4.9, tmax = 5.1, N_conv = 4, plot = False):
        """
        Select data within a specified time range.
    ...
    def Fit_Load(self, tmin, tmax, N_conv=4, plot = False):
        ...
        for data in self.DATA_RAW:
            ...
            x_diff = x_data[index_diff][:-1]
            y_diff = np.convolve(np.diff(y_data[index_diff]), np.ones(2*N_conv + 1)/(2*N_conv + 1), mode='valid')
            x_start = x_diff[np.argmax(y_diff)] + 0.02
            print(f'x_start = {x_start:.3f} s')
            
            # Select data for fitting based on start time
            index_fit = (x_data > x_start) * (x_data < tmax)
            x_data_fit = x_data[index_fit]
            ...
            p0 = [sat_guess, tau_guess, x_start]
            popt, pcov = curve_fit(Load_MOT, x_data_fit, y_data_fit, p0=p0)
            VL, tau, _ = popt  # Fitted parameters
            dVL, dtau, _ = np.sqrt(np.diag(pcov))  # Uncertainties in fit parameters
            
            vc = VL / tau # V / s
            dvc = (dVL/VL + dtau/tau) * vc
    ...
    def SaveParameters_Load(self):
        """
        Save the load fit parameters to a CSV file.
        """
        if len(self.VL) == 0:
            print('No fit has been performed')
            return
        ...
        df.to_csv(self.Date + f'Load_params_B={self.B}.csv')